...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["email"]))) {
        ...
        $sql_check_email = "SELECT Email FROM adi WHERE Email = ?";
    ...
    if (empty(trim($_POST["Contact_No"]))) {
        ...
        $contact_no_err = "Invalid contact number format.";
    } else {
    ...
    if (empty($email_err) && empty($password_err) && empty($contact_no_err) && empty($firstname_err) && empty($lastname_err)) {
        ...
        if (!$mail->send()) {
            ...
            $sql = "INSERT INTO adi (Email, Password, Contact_No, Firstname, Lastname, account_type) VALUES (?, ?, ?, ?, ?, ?)";
